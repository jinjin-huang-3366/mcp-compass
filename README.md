...
4. Enter the task instructions, the existing pushed task branch, the base branch, the pull request title, the local Codex summary, complete task-specific desk-testing guidance, and—when the task matches one unchecked plan entry—the exact `PLANS.md` item text. Desk testing should include ordered commands or actions, expected results, and any steps that could not be run with the reason.
