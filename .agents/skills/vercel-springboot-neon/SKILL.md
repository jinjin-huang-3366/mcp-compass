...
4. Prepare a non-root Spring Boot OCI image that listens on `PORT`. Use an OCI `CMD` for the server command, select Vercel's `Container` framework preset, and explicitly configure a non-privileged project port such as `8080`. Keep database state external, use a pooled Neon connection with a deliberately small application pool, and leave Flyway as the schema owner. Measure cold start and enable Spring lazy initialization when eager startup exceeds Vercel's container initialization window. If a healthy application still narrowly misses the socket deadline, use a minimal TCP startup gate that opens `PORT` immediately and holds connections until Spring listens on a private port; do not fake a healthy HTTP response before initialization completes.
...
9. Run production smoke tests for health, migrations, authenticated scheduler behavior, a representative API request, CORS, and the frontend. Check runtime logs without printing secrets. If the application embeds dotfiles or hidden directories as generated-project assets, verify that Vercel's source upload includes them or store them under visible template names and map them back at generation time.
